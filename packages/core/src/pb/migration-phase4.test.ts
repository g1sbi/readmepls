...
async function makeArticleWithContent(
  pb: PbHandle["pb"],
  user: string,
  title: string,
  body: string,
...
describe("phase-4 migration", () => {
  it("creates a highlight scoped to the user", async () => {
    const { articleId } = await makeArticleWithContent(
      h.pb,
      userId,
      "h1",
      "hello world body",
...
describe("phase-4 search route", () => {
  ...
  beforeAll(async () => {
    ...
    await makeArticleWithContent(
      ...
      aId,
      "ka",
      "a rare kingfisher by the river",
    );
    await makeArticleWithContent(
      h.pb,
      bId,
  ...
  it("escapes HTML in snippets to prevent stored XSS", async () => {
    ...
    const res = await cxssOwner.send("/api/search?q=monarchbutterfly", {
      method: "GET",
    });
...
describe("highlights tenant isolation", () => {
  it("a user cannot list another user's highlights", async () => {
    ...
    const intruder = await authedClient(
      h.url,
      await (async () => {
        const e = `hi${Date.now()}@test.local`;
        await makeUser(h.pb, e);
        return e;
      })(),
    );
...
describe("article_tags isolation", () => {
  it("a user cannot list another user's manual tags", async () => {
    ...
    const tag = await h.pb
      .collection("tags")
      .create({ user: uc.id, name: "secret", slug: "secret" });
    await h.pb.collection("article_tags").create({
      article: articleId,
      tag: tag.id,
      source: "manual",
      confidence: 1,
...
describe("collection_items scoping", () => {
  it("a user cannot read another user's collection items", async () => {
    ...
    await h.pb
      ...
      .create({ collection: col.id, article: articleId, order: 0 });
  ...
  it("deleting an article cascades to its highlights and collection_items", async () => {
    ...
    const remainingHighlights = await h.pb
    ...
    const remainingItems = await h.pb
      .collection("collection_items")
      .getFullList({
