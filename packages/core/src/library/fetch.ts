...
export async function fetchLibraryPage(
  ...
  if (params.favsrc) {
    ...
    effective = {
      ...params,
      source: [...new Set([...params.source, ...favIds])],
    };
  }
  ...
  if (params.sort === "relevance" && rankOrder) {
    ...
    return {
      items: all.slice(start, start + built.perPage),
  ...
  const list = await pb
    .collection("articles")
    .getList(built.page, built.perPage, {
      ...opts,
      sort: built.sort || "-created",
    });
  return {
    items: list.items,
...
export function tallyCollectionCounts(
  ...
  for (const it of items)
    counts.set(it.collection, (counts.get(it.collection) ?? 0) + 1);
...
export async function fetchCollections(
  ...
  const colRows = await pb
    .collection("collections")
    .getFullList({ sort: "name", requestKey: null });
  const collections = colRows.map((c) => ({
    id: c.id,
  ...
  try {
    const rows = await pb
      ...
      .getFullList({ fields: "collection", requestKey: null });
    items = rows.map((r) => ({ collection: r.collection as string }));
  } catch {
    // Degrade to the zero-filled counts already in `items`.
  }
...
export async function fetchFacetOptions(pb: PocketBase): Promise<{
  ...
  return {
    ...
    collections,
    options: deriveFacetOptions(
      artRows as unknown as ArticleFacetRow[],
      favoriteIds,
    ),
  };
}
