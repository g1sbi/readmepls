...
const only = <T extends readonly string[]>(
  vals: string[],
  allowed: T,
): T[number][] =>
  vals.filter((v): v is T[number] =>
    (allowed as readonly string[]).includes(v),
  );
const preset = (sp: URLSearchParams, key: string): DatePreset | null => {
  const v = sp.get(key);
  return v && (DATE_PRESETS as readonly string[]).includes(v)
    ? (v as DatePreset)
    : null;
};
...
export function serializeLibraryParams(p: LibraryParams): URLSearchParams {
  ...
  list("time", p.time);
  list("tag", p.tag);
  list("collection", p.collection);
  list("source", p.source);
  list("lang", p.lang);
  list("author", p.author);
  list("has", p.has);
