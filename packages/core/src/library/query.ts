...
function presetBound(
  ...
  else if (preset === "year" || preset === "older")
...
export function buildLibraryQuery(
  ...
  const groups: string[] = [];
  const orGroup = (parts: string[]) => {
    if (parts.length)
      groups.push(parts.length === 1 ? parts[0]! : `(${parts.join(" || ")})`);
  };
  ...
  if (p.read.length === 0) {
    ...
    orGroup(
      p.read.map((r) =>
        r === "finished"
          ? `(progress >= ${bind(FINISHED_THRESHOLD)} && status != ${bind("archived")})`
          : `status = ${bind(r)}`,
      ),
    );
  }
  ...
  orGroup(
    p.time.map((t) =>
      t === "quick"
        ? `content.read_time < ${bind(5)}`
        : t === "long"
          ? `content.read_time > ${bind(15)}`
  ...
  orGroup(
    p.collection.map(
      (c) => `collection_items_via_article.collection ?= ${bind(c)}`,
    ),
  );
  ...
  orGroup(
    p.has.map((h) =>
      h === "highlights"
        ? `highlights_via_article.id ?!= ${bind("")}`
        : `highlights_via_article.note ?!= ${bind("")}`,
    ),
  );

  if (p.saved) {
    const b = presetBound(p.saved, now);
    groups.push(`created ${b.op} ${bind(b.iso)}`);
  }
  if (p.published) {
    const b = presetBound(p.published, now);
    groups.push(`content.published_at ${b.op} ${bind(b.iso)}`);
  }
...
export function applySearchIds(ids: string[]): {
  expr: string;
