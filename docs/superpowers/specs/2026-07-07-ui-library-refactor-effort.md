...
decides _that_ the 15 hand-rolled primitives in
`apps/web/src/lib/components/ui/` are replaced with shadcn-svelte equivalents.
This document works out the _effort_: starting with the inventory below (what
...
actually builds _on_ are already in place — Svelte `^5.0.0` and `bits-ui
