<script lang="ts">
  ...
  let {
    ...
    onTheme,
    onSignOut,
  }: { theme: Theme; onTheme: (t: Theme) => void; onSignOut: () => void } =
    $props();
  ...
  <div class="themes" role="group" aria-label="theme">
    {#each THEMES as t (t)}
      ...
      <button
        ...
        data-active={theme === t}
        onclick={() => onTheme(t)}
        ><Icon class="icon-sm" aria-hidden="true" /><span class="label"
          >{t}</span
        ></button
      >
    {/each}
...
{#snippet signOutButton()}
  <button type="button" class="signout" onclick={onSignOut}
    ><LogOut class="icon-sm" aria-hidden="true" />sign out</button
  >
{/snippet}
...
<header class="topbar" use:trackTopbarHeight>
  ...
  <nav>
    <a href={resolve("/library")}
      ><Library class="icon-sm" aria-hidden="true" />library</a
    >
  </nav>
  <button
    type="button"
...
<style>
  .topbar {
    ...
    position: sticky;
    top: 0;
    z-index: 20; /* below the reading-progress strip (z-index: 21), which overlays the chrome */
  }
  .brand {
  ...
  .themes button[data-active="true"] {
  ...
  @media (max-width: 640px) {
    ...
    .right {
      display: none;
    } /* moved to bottom nav / menu sheet */
    .menu-btn {
      display: inline-flex;
    }
    /* Keep labels visible inside the menu sheet even on mobile. */
    .sheet-menu .themes button .label {
      position: static;
      width: auto;
      height: auto;
      clip: auto;
