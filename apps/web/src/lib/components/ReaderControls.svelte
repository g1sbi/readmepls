<script lang="ts">
  ...
  let {
    prefs,
    onChange,
  }: { prefs: ReaderPrefs; onChange?: (p: ReaderPrefs) => void } = $props();
  const emit = (patch: Partial<ReaderPrefs>) =>
...
<div class="controls" role="group" aria-label="reading controls">
  <button
    class="seg"
    onclick={() => emit({ size: clampSize(prefs.size - 1) })}
  ...
  <button
    class="seg seg--text"
    onclick={() => emit({ font: prefs.font === "serif" ? "sans" : "serif" })}
  >
    <Type class="icon-sm" aria-hidden="true" />
