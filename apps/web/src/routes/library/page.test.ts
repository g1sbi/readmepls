...
vi.mock("$lib/article/delete.js", () => ({
  deleteArticle: (_pb: unknown, id: string) => del(id),
}));
...
const article = {
  ...
  progress: 0,
  expand: {
    content: { extract_status: "ok", title: "Hello", ai_tags_json: [] },
  },
};
const data = {
  ...
  facets: {
    tags: [],
    collections: [
      { id: "c1", name: "reading list", slug: "reading-list", count: 3 },
    ],
...
describe("library page", () => {
  ...
  it("manages collections from within the filter drawer", async () => {
    ...
    expect(drawer.getByRole("link", { name: /reading list/i })).toHaveAttribute(
      "href",
      "/collections/reading-list",
    );

    await fireEvent.click(
      drawer.getByRole("button", { name: /new collection/i }),
    );
    const input = drawer.getByLabelText(/new collection name/i);
    await fireEvent.input(input, { target: { value: "later reads" } });
    await fireEvent.click(drawer.getByRole("button", { name: /^create$/i }));
  ...
  it("renders the pager and advances to the next page", async () => {
    const multiPageData = {
      ...data,
      page: { items: [article], totalItems: 48, page: 1, perPage: 24 },
    };
